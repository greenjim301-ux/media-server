...
MsSipMsg::MsSipMsg()
	: m_from("From"), m_to("To"), m_callID("Call-ID"), m_cseq("CSeq"),
	  m_contact("Contact"), m_maxForwards("Max-Forwards"), m_userAgent("User-Agent"),
	  m_expires("Expires"), m_subject("Subject"), m_wwwAuthenticate("WWW-Authenticate"),
	  m_authorization("Authorization"), m_date("Date"), m_event("Event"), m_xSource("X-Source")
{
}

string GenNonce()
{
	return GenRandStr(16);
}
...
void BuildSipMsg(const string &fromIP, int fromPort, const string &fromID,
				 ...
				 int cseq, const string &method, MsSipMsg &sipMsg)
{
