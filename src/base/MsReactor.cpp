...
MsReactor::MsReactor(int type, int id)
    : m_index(0), m_type(type), m_id(id), m_exit(false)
{
...
int MsReactor::AddEvent(shared_ptr<MsEvent> evt)
{
    ...
    int ret = epoll_ctl(m_efd, EPOLL_CTL_ADD, evt->GetSocket()->GetFd(),
                        evt->GetEvent());
...
int MsReactor::DelEvent(shared_ptr<MsEvent> evt)
{
    ...
    int ret = epoll_ctl(m_efd, EPOLL_CTL_DEL, evt->GetSocket()->GetFd(),
                        evt->GetEvent());
...
{
    return epoll_ctl(m_efd, EPOLL_CTL_MOD, evt->GetSocket()->GetFd(),
                     evt->GetEvent());
}

int MsReactor::AddTimer(MsMsg &msg, int inter, bool repeat /*= false*/)
{
    return MsTimer::Instance()->AddTimer(shared_from_this(), msg, inter,
                                         repeat);
}
...
void MsReactor::Run()
{
    ...
    shared_ptr<MsEventHandler> evtHandler =
        make_shared<MsNotifyHandler>(shared_from_this());
    shared_ptr<MsEvent> msEvent =
        make_shared<MsEvent>(eventfd, MS_FD_READ, evtHandler);
    ...
    while (!m_exit)
    {
        ...
        if (ret > 0)
        {
            for (int i = 0; i < ret; ++i)
            {
                ...
                {
                    MS_LOG_WARN("index:%d no event",
                                m_eventHandles[i].data.u32);
                }
            }
...
MsReactor::MsNotifyHandler::MsNotifyHandler(
    const shared_ptr<MsReactor> &reactor)
    : m_reactor(reactor) {}
