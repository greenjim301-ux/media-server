...
char *XMLDocument::Identify(char *p, XMLNode **node, bool first) {
	...
	if (XMLUtil::StringEqual(p, xmlHeader, xmlHeaderLen)) {
		...
		// Preserve whitespace pedantically before closing tag, when it's immediately after opening
		// tag
...
XMLNode::XMLNode(XMLDocument *doc)
    : _document(doc), _parent(0), _value(), _parseLineNum(0), _firstChild(0), _lastChild(0),
      _prev(0), _next(0), _userData(0), _memPool(0) {}
...
// ChildElementCount was originally suggested by msteiger on the sourceforge page for TinyXML and
...
char *XMLElement::ParseAttributes(char *p, int *curLineNumPtr) {
	...
	while (p) {
		p = XMLUtil::SkipWhiteSpace(p, curLineNumPtr);
		if (!(*p)) {
			_document->SetError(XML_ERROR_PARSING_ELEMENT, _parseLineNum, "XMLElement name=%s",
			                    Name());
			return 0;
		}
		...
		if (XMLUtil::IsNameStartChar(static_cast<unsigned char>(*p))) {
			...
			if (!p || Attribute(attrib->Name())) {
				...
				_document->SetError(XML_ERROR_PARSING_ATTRIBUTE, attrLineNum, "XMLElement name=%s",
				                    Name());
				return 0;
			}
...
void XMLDocument::SetError(XMLError error, int lineNum, const char *format, ...) {
	...
	TIXML_SNPRINTF(buffer, BUFFER_SIZE, "Error=%s ErrorID=%d (0x%x) Line number=%d",
	               ErrorIDToName(error), static_cast<int>(error), static_cast<unsigned int>(error),
	               lineNum);
...
void XMLPrinter::PushHeader(bool writeBOM, bool writeDec) {
	if (writeBOM) {
		static const unsigned char bom[] = {TIXML_UTF_LEAD_0, TIXML_UTF_LEAD_1, TIXML_UTF_LEAD_2,
		                                    0};
