...
public:
    ...
    void HandleWrite(shared_ptr<MsEvent> evt)
    {
        m_iServer->OnWriteEvent(evt);
    }
...
{
public:
    MsRtspAcceptHandler(shared_ptr<MsReactor> reactor)
        : m_reactor(reactor) {}
    ...
    void HandleRead(shared_ptr<MsEvent> evt)
    {
        ...
        shared_ptr<MsEventHandler> evtHandler = make_shared<MsRtspHandler>(
            m_reactor, dynamic_pointer_cast<MsIRtspServer>(m_reactor));
        shared_ptr<MsEvent> msEvent = make_shared<MsEvent>(
            s, MS_FD_READ | MS_FD_CLOSE, evtHandler);
...
void MsRtspServer::Run()
{
    ...
    shared_ptr<MsEventHandler> evtHandler = make_shared<MsRtspAcceptHandler>(
        shared_from_this());
...
int MsRtspSink::HandleSetup(MsRtspMsg &msg, shared_ptr<MsEvent> evt)
{
    ...
    else
    {
        ...
        char buf[64];
        snprintf(buf, sizeof(buf), "RTP/AVP/TCP;unicast;interleaved=%d-%d",
...
void MsRtspSink::OnStreamInfo(AVStream *video, int videoIdx, AVStream *audio, int audioIdx)
{
    ...
    m_sock->SetNonBlock();
    handler = make_shared<MsRtspHandler>(
        m_reactor, dynamic_pointer_cast<MsIRtspServer>(shared_from_this()));
    ...
    if (m_audio)
    {
            ...
            codecpar->ch_layout.nb_channels > 0 && codecpar->ch_layout.nb_channels < 3)
        {
            ...
            const static int sampleRateTable[] = {96000, 88200, 64000, 48000, 44100,
            ...
            int audioObjectType = (codecpar->profile > 0) ? codecpar->profile + 1 : 2; // Default to AAC-LC
            ...
            // Byte 1: samplingFrequencyIndex (1 bit) | channelConfiguration (4 bits) | frameLengthFlag (1 bit) | dependsOnCoreCoder (1 bit)
            uint16_t config = 0;
...
void MsRtspSink::OnWriteEvent(shared_ptr<MsEvent> evt)
{
    ...
    MS_LOG_INFO("rtsp sink streamID:%s, sinkID:%d all data sent",
                m_streamID.c_str(), m_sinkID);
}

void MsRtspSink::OnCloseEvent(shared_ptr<MsEvent> evt)
{
    this->ActiveClose();
}
