...
class MsMeidaSink {
public:
	MsMeidaSink(const std::string &type, const std::string &streamID, int sinkID)
	    : m_type(type), m_streamID(streamID), m_sinkID(sinkID) {}
	virtual ~MsMeidaSink() { MS_LOG_INFO("media sink %s-%d destroyed", m_type.c_str(), m_sinkID); }
...
public:
	MsMediaSource(const std::string &streamID, int type, int id)
	    : MsReactor(type, id), m_streamID(streamID) {}
